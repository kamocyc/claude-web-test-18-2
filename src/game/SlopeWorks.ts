 ...
 * 土なら 34 度、軟弱層なら 30 度。深い切土ほど横へ広く用地を食う。
 ...
 *   - 費用は押した瞬間に前払い
 *   - **施工班は支保と同じ 1 班**。ただし工期は 0 なので (`Crew.CREW_TIMED`)、
 *     押した瞬間に効く。班を 1 つにまとめてあるのは、工期を戻したときに
 *     「トンネルを急ぐか、法面を押さえるか」が選択になるため
 *   - 色 = 工法 / 中間色 = 手配済みだが未着 (支保のリングと同じ規約。
 *     工期 0 のいまは中間色の状態を通らない)
 *
...
export class SlopeWorks {
  ...
  /**
   * 塗る。費用は前払い。工期 0 なので、効くのも押した瞬間
   * (`crew.push` がその場で `finish` まで走る)。
   * @returns 実際に予約できたか
   */
