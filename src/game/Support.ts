...
/**
 * `hours` は**いまは使われていない**。工期 0 (`Crew.CREW_TIMED`) なので、
 * 押した瞬間に入る。表を残してあるのは工期を戻せるようにするためで、
 * 0 で潰すと「高い支保ほど手間がかかる」という設計意図まで消える。
 *
 * 工期を戻すときの決まりごと: 施工時間は Tunnel.ts の HOURS_TO_FAIL_BASE より
 * 十分短く保つこと。施工班は 1 班しかいない (順番に処理する) ので、長い
 * トンネルを一気に掘ると末端の施工が間に合わずに落ちる。それが狙いの緊張。
